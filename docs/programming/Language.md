...
> 通过lateinit关键字，可以延迟初始化属性，允许将非空类型的属性推迟到后面的代码中进行赋值
...
> 受限的类继承结构，描述一组相关的类， 多用于when判断多个实例
...
> 在java中静态方法定义很简单，直接在方法添加static修饰符即可；但是在Kotlin中有两种方式：它这么做就是为了方便，比如定义工具类时就不需要写一堆static
...
> 像操作变量一样操作函数，将函数作为参数或返回值；提供了更灵活的**函数组合**和**代码复用**方式
>
...
- run    这个**直接对对象**进行初始化并将最后一行作为**返回值**返回
